...
from NITU1305_class import NITU1305_Add


# 執行此命令進行測試
# python -m pytest -v -s

def test_NITU1305(my_fixture):
    ...
    with sync_playwright() as playwright:
        try:
            test= NITU1305_Add(playwright)
            # test = AddInfo(playwright)
            test.navigate()
            test.login('TEST41', 'admin001')

    # ## 新增
            # test.Noscan_apply(personinfo)
            # ## # 要保人資訊新增
            # test.Add(personinfo,product)

    # ##查詢(若要比當前營業日早的建檔，需自行手動變更營業日後申請保單號碼，此案例為7/1建單)
    ...
    # # # # 投資商品新增(傳統型)
            ...
            # # 傳統型受益人
            test.BenefitPeople_NITU1305_NTD(BenefitInfo)
