...
class NITU1305_Add(AddInfo): # 從AddInfo子繼承過來 
    def __init__(self, playwright: Playwright):
        super().__init__(playwright)

        # 投資型商品錄入
    def NITU1305_Add(self, product):
        ...
        # 選擇繳別 0躉繳 -1不定期繳  P.S.# 第一期保費的繳別不會是不定期繳
        if (product["PayIntv"]=="-1"):
            self.page2.frame(name="fraInterface").dblclick("#Amnt")
            # 第二欄繳別
        ...
        # self.page2.frame(name="fraInterface").select_option("select[name=\"codeselect\"]",value="M014")
        self.page2.frame(name="fraInterface").select_option("select[name=\"codeselect\"]", product["InvestTarget"])
    ...
    def BenefitPeople_NITU1305_NTD(self, BenefitInfo):
        # time.sleep(2)
        # print("進入自己的投資方法")
        ...
        self.page6.wait_for_timeout(1000)
